    ...
    DateRangePicker.prototype = {
        ...
        updateInputText: function() {
            if (this.element.is('input')) {
                ...
                $(this.element).data('startDate', this.startDate.format(this.format))
                               .data('endDate'  , this.endDate.format(this.format))
                               .trigger('DatePicker.change')
                ;
                //console.dir($(this.element).data());
        ...
        clickRange: function (e) {
            ...
            if (label == this.locale.customRangeLabel) {
                ...
                $(this.element).val(label)
                               .data('startDate', this.ranges[label][0].format(this.format))
                               .data('endDate'  , this.ranges[label][1].format(this.format))
                               .trigger('DatePicker.change')
                ;
        ...
        updateDateLimit: function(options) {

            this.dateLimit = { days: options.dateLimit };

            this.updateView();
            this.updateCalendars();
        },

        updateRanges: function(options) {
            ...
            $.each(options.ranges, function(index, days) {
                if (days > 7) {
                    ranges['Last '+days+' days'] = [moment().subtract('days', days - 1), moment()]
                }
            });
            ...
            DateRangePicker(this.element, this.options, this.cb);
                    ...
                    if (options.action !== undefined) {
                        ...
                        switch (options.action) {
                            ...
                            case 'updateDates':
                                ...
                                $(this).val($('input[name="daterangepicker_start"]').val()+' - '+$('input[name="daterangepicker_end"]').val());

                                break;

                            case 'updateDateLimit':

                                dp.updateDateLimit(options);

                                break;

                            case 'updateRanges':

                                dp.updateRanges(options);
