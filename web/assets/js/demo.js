...
$(document).ready(function() {
	...
	$('#vehicle-detail-quick-history-tab').trigger('click');
	$('#vehicle-detail-info-tab').hide();
	$('#vehicle-detail-commands-tab').hide();
	$('#vehicle-detail-verification-tab').hide();
	$('.has-popover').hide();
	$('.transition').hide();
	
	var $self = $('#vehicle-li-1735'),
                    ...
                    if (unitId != undefined) {
                        ...
                        if ($self.is('.active')) {   //  if item was just selected
                        	//var method = 'getLastReportedEvent';
                        	//var singleItemEventId = null;
                            var method = '',
                                selectedItems = Array(),
                                singleItemId = '',
                                singleItemEventId = null
                            ;
                            ...
                            if (selectedItems.length >= 1 || ((selectedItems.length == 0) && (! autoOpenDetailPanel))) {  // only get event info if more than one vehicle is currently selected (includes select ALL markers)
                                method = 'getLastReportedEvent';

                                if (selectedItems.length == 1) {   //  only one item is current
                                    singleItemId = selectedItems.attr('id').split('-')[2];
                                    singleItemEventId = selectedItems.data('eventId');
                                    $('#secondary-panel-pagination').data('drawMarker', '');

                                    if (singleItemEventId == '' || singleItemEventId == null) { // unselect previous item if it has no event
                                ...
                                if (selectedItems.length == 2) {
                                    Map.showHideAllLabels(Vehicle.Map.map, true);
                                }
                                ...
                                $.ajax({
                                    ...
                                    success: function(responseData) {
                                        if (responseData.code === 0) {
                                            ...
                                            // Update the Map  (if in vehicle/demo context)
                                            if (Core.Environment.context() == 'vehicle/demo') {
                                                ...
                                                if (method === 'getLastReportedEvent') {	//	create marker only (more than one vehicle is current selected)
                                                    Map.closeInfoWindow(Vehicle.Map.map);

                                                    if (singleItemId != '' && singleItemEventId != null) {
                                                ...
                                                } else {	//	create marker, open info window, and open detail vehicle tab (single vehicle was selected)
                                                    Vehicle.Common.DetailPanel.render(unitdata, function() {
                                                        ...
                                                        if (! $.isEmptyObject(eventdata)) {
                                                            Map.openInfoWindow(Vehicle.Map.map, 'unit', eventdata.latitude, eventdata.longitude, eventdata);
                                                            //$('#vehicle-detail-info-tab').trigger('click');
                                                        } else {
                                                            //$('#vehicle-detail-info-tab').trigger('click', function() {
                        ...
                        } else if (Core.Environment.context() == 'vehicle/demo') {       // else if item was just unchecked

                            /**
                             *
                             * Deselect Vehicle (if in vehicle/demo context)
                             *
                             * */

                            Map.removeMarker(Vehicle.Map.map, unitId);
                            Map.updateMarkerBound(Vehicle.Map.map);
                            ...
                            if (selectedItems.length == 0) {
                                ...
                                var singleItemId = selectedItems.attr('id').split('-')[2],
                                    eventId = selectedItems.data('event-id'),
                                ...
                                Map.showHideLabel(Vehicle.Map.map, singleItemId, false);


                                if ($detailPanelHooks.data('vehiclePk') == singleItemId) {  // if detail panel already contains data for this unit, just open detail panel again
                                    Vehicle.Common.DetailPanel.open(function() {
                                        Map.resize(Vehicle.Map.map);
                                        Map.clickMarker(Vehicle.Map.map, singleItemId);
                                    });
        ...
        initQuickFilters: function() {
            ...
            $quickFilters.click(function() {
                ...
                if ($clickedIcon.is('.active')) {
                    ...
                    // reset to default
                    $('#select-vehicle-attribute-filter').val('All').text('All');
                    ...
                    $('#select-vehicle-attribute-filter').val(vehicle_filtervalue).text(vehicle_filterlabel);
                    ...
                    if (Core.Environment.context() == 'vehicle/demo') {
                        ...
                        Vehicle.Common.SecondaryPanel.fetchFilteredVehicles(true);
                    }
            ...
            initGroups: function() {
                ...
                Vehicle.List.DataTables.vehicleListTable = Core.DataTable.init('vehicle-list-table', 20, {
                    ...
                    'fnServerParams': function (aoData) {
                        ...
                        var vehicle_group_id        = $('#select-vehicle-group-filter').val().trim();
                        var vehicle_state_status    = $('#select-vehicle-attribute-filter').val().trim();
                        var toDay                   = new Date();
                ...
                $(document).on('click', '.col-name a, #new-vehicle-group-modal-trigger', function() {
                    ...
                    if ( ! isNewVehicleGroup) {
                        ...
                        _launchModal({
                            // hide
                            hide: function() {
                                Core.log('hide callback - new');

                            },
                            // hidden
                            hidden: function() {
                                Core.log('hidden callback - new');

                            },
                            // show
                            show: function() {
                                Core.log('show callback - new');


                            },
                            // shown
                            shown: function() {
                                Core.log('shown callback - new');
                    ...
                    function _getVehicleGroupInfo() {
                        ...
                        *//*if ($toggle.text() == 'Show Less Options') {
                        ...
                        if (isNewVehicleGroup) {
                            ...
                            Core.log('_getVeh....NEW');
                        } else {
                            Core.log('_getVeh....EDIT');
                            $editingModalButtons.show();
                            $newModalButtons.hide();
                        }*/

                        Core.log('getting group info')
                                ...
                                if (responseData.code === 0) {
                                    ...
                                    if (! $.isEmptyObject(unitgroupdata)) {
                                        ...
                                        _launchModal({
                                            ...
                                            // show
                                            show: function() {
                                                Core.log('show callback - edit');
                                            },
                                            // shown
                                            shown: function() {

                                                Core.log('shown callback - edit');
                                                ...
                                                $.ajax({
                                                    ...
                                                    success: function(responseData) {
                                                        if (responseData.code === 0) {
                                                            ...
                                                            var vehicleGroupName = $('#vehicle-group-name').text() == 'Not Set' ? 'Default' : $('#vehicle-group-name').text();

                                                            $('#vehicle-group-source').val(vehicleGroupId).text(vehicleGroupName);
                                                            Core.log($('#vehicle-group-name').text());
                                               ...
                                               // $('#vehicle-group-source').closest('.btn-group').find('li').filter('.transfer-group-id-' + vehicleGroupId).find('a').trigger('click');
                                                ...
                                                //Core.log($('#vehicle-group-source').closest('.btn-group').find('.btn-dropdown ~ .dropdown-menu li.transfer-group-id-' + vehicleGroupId+' a'));
    ...
    Common: {
        ...
        SecondaryPanel: {
            ...
            init: function() {
                ...
                $('#vehicle-toggle-all').click(function() {
                    ...
                    $nonActiveItems.each(function(key, value) {
                        ...
                        if ((key == lastIndex) && (unitIds.length > 0)) {
                            $.ajax({
                 ...
                 *
                 * handles showing vehicle on the map and open Detail Panel if only a single vehicle is selected
                 *
                 * */
                $subPanelItems.on('click', 'li', function(event, extraParams) {
                    ...
                    if (unitId != undefined) {
                        ...
                        if ($self.is('.active')) {   //  if item was just selected
                            var method = '',
                                selectedItems = $subPanelItems.find('li').not($self).filter('.active'),     //  get all list items that are currently active (excluding self)
                                singleItemId = '',
                                singleItemEventId = null
                            ;
            ...
            initVehicleSearch: function() {
                ...
                var $vehicleSearch                  = $('#text-vehicle-search');
                var $vehicleSearchGo                = $('#vehicle-search-go');
                var $vehicleGroupFilter             = $('#select-vehicle-group-filter');
                var $vehicleGroupAttributeFilter    = $('#select-vehicle-attribute-filter');
                ...
                $vehicleSearch.on('keyup', function () {
                    ...
                    $('#select-vehicle-group-filter').val('All').text('All');
                    $('#select-vehicle-attribute-filter').val('ALL').text('All');
            ...
            fetchFilteredVehicles: function (drawMarkers, updateQuickFilters, activeUnits)
            {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    data: {
                        vehicle_group_id:       $('#select-vehicle-group-filter').val().trim(),
                        vehicle_state_status:   $('#select-vehicle-attribute-filter').val().trim(),
                        vehicle_listing_length: $('#secondary-panel-pagination a.active').data('value'),
                        vehicle_start_index:    $secondaryPanelPagination.data('vehicleStartIndex'),
                        paging:                 $secondaryPanelPagination.data('paging')
                    },
        ...
        DetailPanel : {
            ...
            render: function(unitdata, callBack) {
                if ((unitdata != undefined) && (typeof(unitdata) == 'object') && (! $.isEmptyObject(unitdata))) {
                   ...
                   $('#vehicle-detail-verification-tab').addClass('draw_table');
                    ...
                    /***************
                     *
                     * Map/Detail Panel Animation (if in vehicle/demo context)
                     *
                     ***************/
                    //setTimeout(function(){
            ...
            {
console.log('demo.js:Vehicle:DetailPanel:basic_render');
               /***************
               *
               * VEHICLE INFO
               *
            ...
            open: function(callBack) {
                ...
                if (Core.Environment.context() == 'vehicle/demo') {
                    ...
                    $mapDiv.animate({
                        //'height': '400px'
                        //'height': (Core.Viewport.contentHeight*0.75)+'px'
                        'height': (Core.Viewport.contentHeight-parseInt($detailPanel.css('height'))-35)+'px'
                    }, 300, function() {
            ...
            initInfoTab: function() {
                ...
                var $vehicleName               = $('#vehicle-name'),
                    ...
                    $vehicleGroupFilterSelect  = $('#select-vehicle-group-filter'),
...
console.log('demo.js:Vehicle:DetailPanel:initInfoTab:$vehicleOdometer:Core.FormElementChanged');
                    if (Core.Environment.context() == 'vehicle/demo' || Core.Environment.context() == 'vehicle/list') {
...
console.log('demo.js:Vehicle:DetailPanel:initInfoTab:$vehicleOdometer:Core.FormElementChanged:totalMileage updated');
                            }
                        }
                    }
                });
                ...
                $vinDecoder.click(function() {
                    var vin = $('#vehicle-vin').text() || '',
                        $container = (Core.Environment.context() == 'vehicle/demo') ? $('#detail-panel') : $('#modal-vehicle-list'),
            ...
            initCommandsTab: function() {
                ...
                /**
                 *
                 * Clicking Command Buttons: starter enable/disable reminder on/off
                 *
                 * */
                $('body').on('click', '#popover-starter-enable-confirm, #popover-starter-disable-confirm, #reminder-on-button, #reminder-off-button', function() {
                    var id = $(this).attr('id')
                        ,action = ''
                        ,unitId = ''
                        ,responseElSelector
                        ,progressBarSnippet = '<div class="progress progress-striped active"><div class="progress-bar progress-bar-warning" role="progressbar" aria-valuenow="100" aria-valuemin="0" aria-valuemax="100" style="width: 100%"></div></div>'

                    switch (id) {
                        case 'popover-starter-enable-confirm':
                            action = 'starter_enable';
                            responseElSelector = '#starter-enable-response';
                            break;
                        case 'popover-starter-disable-confirm':
                            action = 'starter_disable';
                            responseElSelector = '#starter-disable-response';
                            break;
                        case 'reminder-on-button':
                            action = 'reminder_on';
                            break;
                        case 'reminder-off-button':
                            action = 'reminder_off';
                            break;
                        default:
                            break;
                    ...
                    if (Core.Environment.context() == 'vehicle/list') {
                        unitId = $('#detail-panel').find('.hook-editable-keys').eq(0).data('vehiclePk');
                    }
                    ...
                    if (action != '' && unitId != '') {
                        ...
                        $.ajax({
                            ...
                            success: function(id, unitId, action, responseElSelector){ return function(responseData) {

                                var interval,count,limit,timeoutFn
                                ...
                                if (responseData.code === 0) {
                                    ...
                                    timeoutFn = function(){
                                        count++
                                        if (count > limit){
                                            $(responseElSelector).text('Timed out waiting for response.')
                                            return
                                        }
                                        $.ajax({
                                            ...
                                            success: function(responseData) {
                                                if (responseData.code === 0) {
                                                    $(responseElSelector).text(responseData.message)
                                                    _getCurrentLocation()
                                                } else {
                                            ...
                                            error: function(){
                                                $(responseElSelector).text('unknown error')
                                            }
                                        })
                                    }

                                    if (_timeouts[action]) clearTimeout(_timeouts[action]);
                                    $(responseElSelector).html(responseData.message+'<br />Waiting for response...'+progressBarSnippet)
                                    _timeouts[action] = setTimeout(timeoutFn, interval*1000);

                                } else {
                                    $(responseElSelector).text(responseData.validation_error || responseData.message || 'unknown error')
                                }
                ...
                $('body').on('click', '#popover-starter-enable-retry, #popover-starter-disable-retry', function() {
                    var id = $(this).attr('id')
                        ,action
                    switch (id) {
                                        ...
                                        dragend: function(data) {
                                            ...
                                            Map.reverseGeocode(Vehicle.Map.map, data.latitude, data.longitude, function(data1) {
                                                if (data1.success == 1) {
                                                    ...
                                                    //Core.log(data1.latitude + ' / ' + data.longitude);
